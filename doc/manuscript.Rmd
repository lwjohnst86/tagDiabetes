...
The primary outcome variables for this analysis were 1/HOMA-IR, ISI, IGI/IR, and
...
only quantified on a portion of the 6-year visit (n = ~120 {{ find true number
}}). Pearson correlation coefficients were computed to assess the relationships
of individual TAGFA with other continuous variables.
...
cor_pls_igiir <- calc_pred_corr(pls_igiir_t, pls_igiir_t$test_data)
...
measures are presented given the low predictability.

The PLS results for the insulin sensitivity (`r fig_pls`) revealed a clustering 
of the FA 14:0, 14:1n-7, 16:0, and 16:1n-7. These TAGFA loaded strongly (meaning
the FA fell between the dashed and solid circles) and negatively on 1/HOMA-IR
...
testing set. The predictive ability of the PLS model, determined by correlating
the predicted outcome values with the actual values, were good (1/HOMA-IR: 
`r cor_pls_homa$r_p`; ISI: `r cor_pls_isi$r_p`).
