...
`r cite_sf('consort')`. Metabolic characterization, anthropometric measurements,
...
individual TAGFA with other continuous variables.
...
graph [@Greenland1999a] recommendations (see `r cite_st('dag')`), and from 
QIC {{citation?}}. `r cite_st('qic')` shows the covariates compared using QIC.
...
less than 10 between the models, suggesting similar fit. As such, we selected
...
environment (all TAGFA), partial least squares regression (PLS) {{citation}} was
used to identify the patterns of TAGFA composition against insulin sensitivity
and beta-cell function. 
For more detailed explanation of GEE and PLS, please see
