...
the dysregulation of metabolic function and subsequent negative health outcomes {{cite}}.
It is a component of the metabolic syndrome [@Alberti2009], a strong risk factor for
cardiovascular disease [@DAgostino2004a], and has been shown to contribute to the pathogenesis
of diabetes {{cite}}. The measurement of circulating triacylglcerides (TAG) is commonly done
...
components of a feedback system {{cite}}. Greater resistance to insulin in both the liver
...
beta-cells, and thus continuing the cycle {{cite}}. Given the complexity and temporal
...
We ran sensitivity analyses to identify which covariate attenuated the beta-cell
function associations from the unadjusted model. We found that including waist
circumference in the model lead to the attenuation of associations, shown in 
`r cite_st("gee_adj_nowaist")`. A tabular presentation of the GEE results is
shown in `r cite_st("gee_unadj")` for unadjusted models and 
`r cite_st("gee_adj")` for adjusted models.
...
fat stores and consequently increasing the amount of TAGFA in circulation {{cite}}. The higher
concentration of circulating 14 and 16 carbon fatty acids may then expose
tissues to greater lipotoxicity, for instance from palmitic acid (16:0), which
is well-known to have harmful effects on tissues [@Riserus2008a; @Iggman2010a].
...
feedback mechanism between TAGFA and insulin sensitivity {{rephrase after exploring time interaction}}.
...
composition such as dietary intake {{cite?}}.
...
subject-specific estimates in mixed effects models. GEE is well suited and commonly
...
outcome measures. {{NEFA comment for inclusion here}}
...
No prevalent diabetes cases were included in the PLS
analysis. 
