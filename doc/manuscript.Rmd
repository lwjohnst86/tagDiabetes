...
We found that in an at-risk for diabetes Canadian population of mainly female
and European ancestry, the specific TAGFA composition was strongly associated
with insulin sensitivity and moderately associated with beta-cell function. In
particular, TAGFA myristic acid (14:0), 7-tetradecenoic acid {{ confirm }}
(14:1n-7), palmitic acid (16:0), and palmitoleic acid (16:1n-7) all strongly and
negatively predicted lower insulin sensitivity. {{confirm}} While most TAGFA
were not associated with beta-cell function, {{confirm after checking models}}
two fatty acids, 16:0 and 18:1n-7, were associated negatively and positively,
respectively, with both measures of beta-cell function. We also found that four
TAGFA (14:0, 14:1n-7, 16:0, 16:1n-7) clustered together, and that also strongly 
predicted lower insulin sensitivity. These four fatty acids are also involved in
the DNL of refined and simple carbohydrates {{cite}}. Our results, which are in
agreement with the current literature, suggest that higher intakes of simple
carbohydrates or higher activity of DNL may increase the risk for diabetes,
likely through worsening insulin sensitivity.
...
multifacetted {{?}} and as such there may be some residual confounding we were
...
Since TAGFA were only quantified at the baseline visit and as such we cannot
...
integration of the glucose and fatty acid metabolism pathways, as well as uses
GEE modeling appropriately {{? another word}}.

PLS is a well-established technique for constructing predictive models
of high dimensionality data structures (i.e. fatty acid composition), however a
...
we still observe concordant results between the PLS and GEE analyses {{ add more }}.

Even with these limitations, our study also has several major strengths,
including the longitudinal design and the rigorous statistical techniques and
methods applied in analyzing the data. These statistical techniques are
...
comprehensive variable measurements at each collection visit, as well as having
both concentration and mol% data for the fatty acids.

## Interpretation and previous literature

Previous research has shown that carbohydrate intake increases DNL {{ cite }}. In
...
link between higher estimated DNL and an increased risk for diabetes {{ cite }}
as well as studies showing higher refined carbohydrates (e.g. added sugars) and
a higher risk for diabetes {{cite}}. Our study extends {{adds to? confirms?}}
these findings on DNL by showing that TAGFA with 14 to 16 carbons all strongly
predicted lower insulin sensitivity. While these FA also had a significant
association with beta-cell function, the results were not as strong as with the
IR. {{ why is it that DNL increases risk? }}

To our knowledge, no longitudinal study has examined the role of the composition
of the TAGFA fraction on detailed OGTT-derived metabolic measures using 
statistical techniques designed to adequately analyze ... {{shots fired? 
necessary?}}.

To our knowledge, two prospective studies have been published that had similar
research objectives as the present analysis. One was a nested case-control
cohort (n=189 cases and n=189 controls) of the Framingham offspring study
[@Rhee2015a], which found that TAGFA composition with a lower carbon chain and less double
bonds (e.g. 14:0, 16:0) had a higher OR {{confirm}} for diabetes after 12-years
while higher carbon chain and more double bond TAGFA had a lower OR for
diabetes. This pattern of TAGFA with diabetes risk remained
the same when they examined HOMA-IR cross-sectionally at the baseline visit. The
other study was a prospective cohort of males in Finland [@Lankinen2015a], of
which they had TAGFA data on 831 participants after 6-years from the baseline
visit. This cohort has similar outcomes as PROMISE, however, OGTT data was only
available at the 6-year visit. A beta-cell function measure was computed, but
only included calculations up to 30 minutes of the OGTT. As with our results,
they found a larger effect size for insulin sensitivity compared to the
beta-cell function. The specific TAGFA that associated with the metabolic
measures were similar between studies, except they did not find {{confirm}}

In addition, the Framingham offspring study [@Rhee2011a] did not have data on
beta-cell function while the Finnish cohort [@Lankinen2015a] used {{ how do I
say this politely Tony? They're methods and article was ... questionable... }}
simple statistical techniques to analysis the longitudinal data, which do not
adequately take into consideration auto-correlation of longitudinal values and
the high dimensionality of the TAGFA composition data structure.

A smaller study (n=16) of mostly females using highly detailed lipodomic
techniques found a positive correlation between total esterified (of which TAGFA
make up the majority) 16:0, 16:1n-7, and 18:1n-9 with HOMA-IR [@Kotronen2009a].

Limitations with estimated DNL or measured DNL in prev studies.

Several studies have been conducted that link greater hepatic DNL with the 14
and 16 carbon FA. A short, small feeding trial (n=24) was conducted to identify
which biomarkers most accurately reflected DNL [@Lee2015a]. The study found that
16:1n-7, directly measured DNL using isotopes, and liver fat were all highly
correlated with each other, suggesting that 16:1n-7 is a good biomarker for
hepatic DNL.
In another small (n=14) feeding trial, meal type (high fat vs low fat) was
tested to determine the effect on DNL and TAGFA composition [@Wilke2009a]. They
found that 14:0, 16:0, 16:1, and 18:2 were higher in the low fat (high
carbohydrate) group. These fatty acids are similar to the fatty acids we found
that clustered together, suggesting {{another word}} that these fatty acids are
indicative of a higher carbohydrate diet.
Suggesting {{expand/synth}} that greater hepatic fat may also contribute to greater DNL,
increasing the 14 and 16 carbon FA in circulation, potentially exposing tissues
to greater lipotoxicity, resulting in further hepatic dysfunction and continuing
the cycle.
...
{{ hard to disentangle reverse causality here }}
...
within the liver, circulating NEFA, and through DNL {{cite}}. Therefore, the
TAGFA composition reflects not just dietary intake but also stored fatty acids
in adipose tissue and the DNL capacity of the individual. During fasting, a
large portion of fatty acids bound together as TAG are obtained from circulating
NEFA. A study examining this found that NEFA contributed 60-80% of newly synth
VLDL-TAG during fasting and ~40% during fed while dietary fatty acids
contributed ~25% to newly synth VLDL-TAG {{fasting or fed?}} [@Barrows2006a].
While the NEFA fraction is much smaller than the TAG fraction even though it
contributes substantially to the TAG fraction, VLDL-TAG is not cleared
...
why there is bigger TAG pool vs NEFA}}. In an analysis of the NEFA fraction in
the PROMISE cohort with the OGTT-derived outcomes (currently unpublished)
{{correct word for this??}}, we found that total NEFA, but not the specific
composition associated with beta-cell function and not with insulin resistance.
While NEFA may contribute substantially to the TAG fraction, there may be
something unique about the specific composition of the TAGFA fraction that may
allow it to enter insulin sensitive tissues more easily than NEFA. It may be
that VLDL receptors allow FA to transition to these tissue compared to NEFA
bound to albumin {{ confirm and cite }}, and thus able to exert potential
lipotoxic effects. {{ comment on comparing models with and without NEFA }}
...
association with cis-vaccenic acid. {{ continue? }}
...
{{ commment on null interactions with time }}
- Challenge longitudinally since muscle IR can promote DNL + TAG Flannery2012a
Since no interaction, no loop? Not enough sample, already IR? maybe?
...
Prospective cohorts:
    - Chien K, et al. A prediction model for type 2 diabetes risk among Chinese people. Diabetologia. 2009;52(3):443–450.
    - D’Agostino RB Jr, Hamman RF, Karter AJ, Mykkanen L, Wagenknecht LE, Haffner SM. Cardiovascular disease risk factors predict the development of type 2 diabetes: the insulin resistance atherosclerosis study. Diabetes Care. 2004;27(9):2234–2240.
    - Schulze MB, et al. Use of multiple metabolic and genetic markers to improve the prediction of type 2 diabetes: the EPIC-Potsdam Study. Diabetes Care. 2009;32(11):2116–2119.
