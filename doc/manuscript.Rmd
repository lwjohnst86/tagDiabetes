...
The DAG structures to understand potential confounding, shown in `r cite_sf("dag_is")`
and `r cite_sf("dag_bcf")`, were processed by the DAGitty software
[@Textor2011a;@Shrier2008a] to generate the recommended adjustments.
The final
GEE model (M6; seen in `r cite_st('qic')`) was adjusted for years since baseline, WC, baseline age, ethnicity,
...
correlation with clinical TG (r=`r fa_cor_mol$rng$TAG[2]`) while all other fatty
acids had a negative association (r=`r fa_cor_mol$rng$TAG[1]`). In particular,
