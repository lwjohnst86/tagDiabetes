...
Clusters of fatty acids in serum triacylglyceride fraction associate with the
...
**Aims/hypothesis:**
Although elevated serum triacylglyceride (TG) is a well-described risk factor for
type 2 diabetes (T2D), few data are available regarding the role of the specific fatty
acid (FA) composition within serum TG (TGFA) in the pathogenesis of T2DM. Our aim, therefore, was to
examine longitudinal associations of TGFA with insulin sensitivity (IS) and
beta-cell function.
...
quantified TGFA composition. Generalized estimating equations (GEE) adjusted for
...
GEE models, four TGFA (14:0, 16:0, 14:1n-7, 16:1n-7 as mol%) had
...
analysis indicated four TGFA (14:0, 16:0, 14:1n-7, 16:1n-7) that are markers of de novo lipogenesis (DNL)
clustered together and strongly predicted
lower IS. These four TGFA also correlated highly (r>0.4) with clinically
measured TG.

**Conclusions**:
We found that higher proportions of a cluster of four DNL TGFA 
...
However, clinically measured TG is limited as it
represents the full fatty acid spectrum within the TG fraction as a summary
...
outcomes [@Forouhi2014a; @Ma2015a; @Johnston2016a; @Johnston2017b], however there are relatively
few studies that have explored the impact of the fatty acid composition in the TG fraction [@Rhee2011a; @Lankinen2015a].
...
TG and its composition with the pathophysiological factors underlying diabetes, 
particularly beta-cell function. Our objective was to examine the 
longitudinal role of the specific composition of the serum TG fraction on 
OGTT-derived measures of insulin sensitivity and beta-cell function compared to 
clinically measured TG in a Canadian population at risk for diabetes.
...
fig_gee_unadj <- fig('gee_unadj', 'Time-adjusted GEE models of the association of the triacylglycerol fatty acids (mol% and nmol/mL) and total clinically-measured TG with insulin sensitivity and beta-cell function outcomes using the 6 year longitudinal data from the PROMISE cohort. X-axis values represent a percent difference in the outcome per SD increase in the fatty acid. P-values were adjusted for the BH false discovery rate, with the largest dot representing a significant (p<0.05) association.')
fig_gee_adj <- fig('gee_adj', 'Fully-adjusted GEE models of the association of the triacylglycerol fatty acids (mol% and nmol/mL) and total clinically-measured TG with insulin sensitivity and beta-cell function outcomes using the 6 year longitudinal data from the PROMISE cohort. Variables controlled for were follow-up time, waist circumference, baseline age, ethnicity, sex, ALT, physical activity, and total NEFA. X-axis values represent a percent difference in the outcome per SD increase in the fatty acid. P-values were adjusted for the BH false discovery rate, with the largest dot representing a significant (p<0.05) association.')
...
individual TGFA with other continuous variables. Correlations were also
computed and hierarchical clustering analysis was conducted for TGFA against each other.
...
sex, ALT, MET, and total NEFA. The TGFA, total NEFA, sex, ethnicity, and
...
correlations with WC (r=`r fa_cor$rng$Waist[2]`). There were also moderate
...
correlation with clinical TG (r=`r fa_cor_mol$rng$TG[1]`) while all other fatty
acids had a negative association (r=`r fa_cor_mol$rng$TG[2]`). In particular,
...
associations with HOMA2-%S and ISI (between `r gmest1$outcome_b$rng[1]` and 
...
and 22:5n-3) predicting between `r gmest1$outcome_b$rng[2]` and 
...
TGFA loaded strongly. In the second component, 18:1n-9 and 18:1n-7 loaded postively 
...
higher carbon chain and more double bond TGFA had a lower risk for diabetes.
...
allowed us to identify clusters of TGFA. In another study of a much
