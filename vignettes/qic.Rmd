...
prep_qic <- project_data %>%
    prep_gee_data() %>% 
    dplyr::select(
        SID, VN, MonthsFromBaseline, lISSI2, lISI, TotalTG, TotalNE, Sex, Ethnicity, MET, ALT, BaseAge,
...
M0_mfb <- geepack::geeglm(
    lISI ~ TotalTG + MonthsFromBaseline, data = qic_is,
    id = SID, family = stats::gaussian,
    corstr = 'ar1'
    )
